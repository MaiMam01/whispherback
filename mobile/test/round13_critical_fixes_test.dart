...
void main() {
  group('Round 13-A — cross icon is wired to dismissPlayer not stop', () {
    ...
    test(
        ...
        'safe `_audio.stop()` path (which under the hood calls `stopClip`, '
        'not the OEM-risky top-level `audio_service.stop()`)', () {
      final src =
      ...
      final body = src.substring(dismissIdx, dismissIdx + 2500);
      expect(
        body,
        contains('await _audio.stop();'),
        reason: 'dismissPlayer must call _audio.stop which routes to '
            'stopClip — the hardened path that skips super.stop.',
      );
      expect(
        body,
  ...
  group('Round 13-B — schedule conflict check no longer false-positives', () {
    test(
      ...
      // The new 1-minute start-time gate must be present.
      expect(
        src,
        contains('final delta = (existingStartMin - newStartMin).abs();'),
        reason: 'The new check compares start times in minutes since '
            'midnight and only flags conflicts within 1 minute.',
      );
      expect(
        src,
        contains('return delta < 1;'),
        reason: 'The 1-minute tolerance must be the only conflict '
            'criterion (the runtime engine handles real overlap via '
            '`_slotTakenByOtherSchedule`).',
      );
    });
  ...
  group('Round 13-C — engine re-syncs notifications every 30 seconds', () {
    test(
      ...
      expect(
        src,
        contains('static const _notificationSyncCadence = Duration(seconds: 30);'),
        reason: 'The sync cadence must be 30s so it matches the schedule '
            'page\'s own 30s countdown re-render timer.',
      );
    });
